...
bool XMLHandler::update(string path1, string path2, string property_name, string replace_value){
	...
	for (xml_node<> *child = root->first_node(); child; child = child->next_sibling())
	{
		...
		{
			string att_name = child_attribute->name();
			string att_value = child_attribute->value();
			if(att_name.compare("name")==0 && att_value.compare(property_name) == 0){
				config_node = child;
				break;
			}
		}

		if(config_node!=NULL) break;
	}

	if(config_node==NULL){
		config_node = doc.allocate_node(node_element, "property");
		xml_attribute<> *attr_name = doc.allocate_attribute("name", property_name.c_str());
		xml_attribute<> *attr_type = doc.allocate_attribute("type", "String");
		xml_attribute<> *attr_value = doc.allocate_attribute("value", replace_value.c_str());
		config_node->append_attribute(attr_name);
		config_node->append_attribute(attr_type);
		config_node->append_attribute(attr_value);
		root->append_node(config_node);
	}else{
		xml_attribute<> *attribute = config_node->first_attribute("value");
